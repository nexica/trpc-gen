...
export class ${model.name}Service {
    ...
    async findFirst(data: z.infer<typeof ${model.name}FindFirstArgsSchema>): Promise<z.infer<typeof ${model.name}Schema> | null> {
        return this.${model.name.toLowerCase()}Repo.findFirst(data)
    }

    async findMany(data: z.infer<typeof ${model.name}FindManyArgsSchema>): Promise<z.infer<typeof ${model.name}Schema>[]> {
        return this.${model.name.toLowerCase()}Repo.findMany(data)
    }

    async findUnique(data: z.infer<typeof ${model.name}FindUniqueArgsSchema>): Promise<z.infer<typeof ${model.name}Schema> | null> {
        return this.${model.name.toLowerCase()}Repo.findUnique(data)
    }

    async create(data: z.infer<typeof ${model.name}CreateArgsSchema>): Promise<z.infer<typeof ${model.name}Schema>> {
    ...
    async update(data: z.infer<typeof ${model.name}UpdateArgsSchema>): Promise<z.infer<typeof ${model.name}Schema>> {
    ...
    async upsert(data: z.infer<typeof ${model.name}UpsertArgsSchema>): Promise<z.infer<typeof ${model.name}Schema>> {
        return this.${model.name.toLowerCase()}Repo.upsert(data)
    }

    async delete(data: z.infer<typeof ${model.name}DeleteArgsSchema>): Promise<z.infer<typeof ${model.name}Schema> | null> {
